...
def process_video(video_path, output_dir, skip_frames=0, method="裁剪"):
    ...
    # 获取总帧数（可能出现无法精确获取的情况）
    total_frames = int(cap.get(
        cv2.CAP_PROP_FRAME_COUNT)) if cap.isOpened() else 0
    ...
    while True:
        ...
        print(f"生成第{saved_frame_count}帧")
        frame_path = os.path.join(output_dir, f"{saved_frame_count}.jpg")
...
if __name__ == "__main__":
    process_video("cat.m4s", "./frames", skip_frames=100, method="裁剪")
